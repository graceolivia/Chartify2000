...
class ValidatorTests: XCTestCase {
    func testInvalidStitchShouldThrowError() throws {
        ...
        expect { try InputValidator().validateInput(pattern: testPattern) }.to(throwError(err))
    }

    func testEmptyRowShouldThrowError() throws {
        let testPattern = ["p1 p1", ""]
        expect { try InputValidator().validateInput(pattern: testPattern) }.to(throwError(InputError.emptyRow))
    }
    ...
    func testInvalidStitchCountShouldThrowError() throws {
        ...
        expect { try InputValidator().validateInput(pattern: badCountPattern) }.to(throwError(badCountError))
    }

    func testValidPatternShouldReturnMetaData() throws {

        let result = try InputValidator().validateInput(pattern: ["p1 p1", "p1 p1"], knitFlat: false)
    ...
    func testValidPatternKnitFlatShouldReturnMetaData() throws {
        let result = try InputValidator().validateInput(pattern: ["k1 p1", "p1 k1"], knitFlat: true)
        let expectedResult = [
            RowInfo(
                row: ["k1", "p1"],
                rowIndex: 0,
